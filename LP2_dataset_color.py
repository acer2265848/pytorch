        ...
        if is_cross==False:
            ...
            for i in x.batch.unique():
                ...
                if np.all(df.status==1):
                    path=pre_pat+"LP2"+whole+"/"+method+"/train/status_1/"
                elif np.all(df.status==2):
                    path=pre_pat+"LP2"+whole+"/"+method+"/train/status_2/"
                elif np.all(df.status==3):
                    path=pre_pat+"LP2"+whole+"/"+method+"/train/status_3/"
                elif np.all(df.status==4):
                    path=pre_pat+"LP2"+whole+"/"+method+"/train/status_4/"
                else:
                    path=pre_path+"LP2"+whole+"/"+method+"/train/status_5/"
            ...
            status_list=["status_1","status_2","status_3","status_4","status_5"]
            for k in status_list:
                status_all  = pd.read_json("LP2_"+k+"_validation"+whole+".json")
                x.append(status_all)
            x = pd.concat(x,axis=0,ignore_index=True)
            x=x.sample(frac=1)
            ...
            for i in x.batch.unique():
                ...
                lst = list()
                if np.all(df.status==1):
                    path=pre_pat+"LP2"+whole+"/"+method+"/validation/status_1/"
                elif np.all(df.status==2):
                    path=pre_pat+"LP2"+whole+"/"+method+"/validation/status_2/"
                elif np.all(df.status==3):
                    path=pre_pat+"LP2"+whole+"/"+method+"/validation/status_3/"
                elif np.all(df.status==4):
                    path=pre_pat+"LP2"+whole+"/"+method+"/validation/status_4/"
                else:
                    path=pre_path+"LP2"+whole+"/"+method+"/validation/status_5/"
            ...
            for k in status_list:
                for r in cross:
                    status_all  = pd.read_json("LP2_"+k+whole+"_cross_"+r+".json")
                    x.append(status_all)
            x = pd.concat(x,axis=0,ignore_index=True)
            ...
            for i in x.batch.unique():
                ...
                lst = list()
                if np.all(df.status==1):
                    path=pre_path+"LP2_img_cross"+whole+"/"+method+"/"+str(df.iloc[0].cross)+"/status_1/"
                elif np.all(df.status==2):
                    path=pre_path+"LP2_img_cross"+whole+"/"+method+"/"+str(df.iloc[0].cross)+"/status_2/"
                elif np.all(df.status==3):
                    path=pre_path+"LP2_img_cross"+whole+"/"+method+"/"+str(df.iloc[0].cross)+"/status_3/"
                elif np.all(df.status==4):
                    path=pre_path+"LP2_img_cross"+whole+"/"+method+"/"+str(df.iloc[0].cross)+"/status_4/"
                else:
                    path=pre_path+"LP2_img_cross"+whole+"/"+method+"/"+str(df.iloc[0].cross)+"/status_5/"
            ...
            status_list=["status_1","status_2","status_3","status_4","status_5"]
            for k in status_list:
                status_all  = pd.read_json("LP2_"+k+whole+"_cross_"+str(cross_number_for_test)+".json")
                x.append(status_all)
            x = pd.concat(x,axis=0,ignore_index=True)
...
print("LP2 color 1 channel Function setting done")
