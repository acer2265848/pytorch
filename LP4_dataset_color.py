        ...
        if is_cross==False:
            ...
            for i in x.batch.unique():
                ...
                if np.all(df.status==1):
                    path=pre_pat+"LP4"+whole+"/"+method+"/train/status_1/"
                elif np.all(df.status==2):
                    path=pre_pat+"LP4"+whole+"/"+method+"/train/status_2/"
                elif np.all(df.status==3):
                    path=pre_pat+"LP4"+whole+"/"+method+"/train/status_3/"
            ...
            status_list=["status_1","status_2","status_3"]
            for k in status_list:
                status_all  = pd.read_json("LP4_"+k+"_validation"+whole+".json")
                x.append(status_all)
            x = pd.concat(x,axis=0,ignore_index=True)
            x=x.sample(frac=1)
            ...
            for i in x.batch.unique():
                ...
                lst = list()
                if np.all(df.status==1):
                    path=pre_pat+"LP4"+whole+"/"+method+"/validation/status_1/"
                elif np.all(df.status==2):
                    path=pre_pat+"LP4"+whole+"/"+method+"/validation/status_2/"
                elif np.all(df.status==3):
                    path=pre_pat+"LP4"+whole+"/"+method+"/validation/status_3/"
            ...
            for k in status_list:
                for r in cross:
                    status_all  = pd.read_json("LP4_"+k+whole+"_cross_"+r+".json")
                    x.append(status_all)
            x = pd.concat(x,axis=0,ignore_index=True)
            ...
            for i in x.batch.unique():
                ...
                lst = list()
                if np.all(df.status==1):
                    path=pre_path+"LP4_img_cross"+whole+"/"+method+"/"+str(df.iloc[0].cross)+"/status_1/"
                elif np.all(df.status==2):
                    path=pre_path+"LP4_img_cross"+whole+"/"+method+"/"+str(df.iloc[0].cross)+"/status_2/"
                elif np.all(df.status==3):
                    path=pre_path+"LP4_img_cross"+whole+"/"+method+"/"+str(df.iloc[0].cross)+"/status_3/"
            ...
            status_list=["status_1","status_2","status_3"]
            for k in status_list:
                status_all  = pd.read_json("LP4_"+k+whole+"_cross_"+str(cross_number_for_test)+".json")
                x.append(status_all)
            x = pd.concat(x,axis=0,ignore_index=True)
...
print("LP4 color 1 channel Function setting done")
